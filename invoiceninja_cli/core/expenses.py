"""Expense resource operations for InvoiceNinja."""

from __future__ import annotations

from typing import Any, Dict, List, Optional

from .session import InvoiceNinjaSession

_ENTITY = "expenses"


class ExpenseResource:
    """CRUD + bulk operations on InvoiceNinja expenses."""

    def __init__(self, session: InvoiceNinjaSession):
        self.session = session

    def list(
        self,
        page: int = 1,
        per_page: int = 20,
        filter: Optional[str] = None,
        client_id: Optional[str] = None,
        vendor_id: Optional[str] = None,
        all_pages: bool = False,
        **kwargs: Any) -> dict:
        """List expenses with optional filters."""
        params: Dict[str, Any] = {"page": page, "per_page": per_page, "include": "client,vendor"}
        if filter:
            params["filter"] = filter
        if client_id:
            params["client_id"] = client_id
        if vendor_id:
            params["vendor_id"] = vendor_id
        params.update(kwargs)
        if all_pages:
            return self.session.get_all_pages(_ENTITY, params)
        return self.session.get(_ENTITY, params=params)

    def get(self, id: str) -> dict:
        """Fetch a single expense by ID."""
        return self.session.get(f"{_ENTITY}/{id}")

    def create(self, data: Dict[str, Any]) -> dict:
        """Create a new expense.

        Expected data keys:
            vendor_id (str): Associated vendor ID.
            client_id (str): Associated client ID (optional).
            amount (float): Expense amount.
            date (str): Expense date YYYY-MM-DD.
            category_id (str): Expense category ID (optional).
            public_notes (str): Billable notes.
            private_notes (str): Internal notes.
        """
        return self.session.post(_ENTITY, data=data)

    def update(self, id: str, data: Dict[str, Any]) -> dict:
        """Update an existing expense."""
        return self.session.put(f"{_ENTITY}/{id}", data=data)

    def delete(self, id: str) -> dict:
        """Delete an expense by ID."""
        return self.session.delete(f"{_ENTITY}/{id}")

    def archive(self, ids: List[str]) -> dict:
        """Archive one or more expenses."""
        return self.session.bulk(_ENTITY, "archive", ids)

    def restore(self, ids: List[str]) -> dict:
        """Restore one or more archived expenses."""
        return self.session.bulk(_ENTITY, "restore", ids)
